...
permalink: /fzu_cs_course/computer-system-architecture/outline-map/
---
# 第一章 计算机系统结构概论
...
2. 软件实现：
   - 优点: 灵活性好、占用内存多、易设计、可改性强、适应性强、设计周期短
   - 缺点: 速度慢、复制费用低；
...
6. 采用高速缓冲存储器 Cache
   - 分别设置指令Cache 和数据 Cache ，分别存放指令和数据。保证向指令流水线不间断地输送指令和存取数据，提高流水线效率
...
| **交换置换**     | 实现二进制地址编号中**第0位**位置不同的输入端和输出端之间的连接 (相邻奇偶交换)<br />*实际上就是$Cube_0$* | $EX(p_{n-1} p_{n-2}…p_1 p_0)=\\p_{n-1} p_{n-2}…p_1\textcolor{red}{\overline{p_0}}$ | <img src="./系统结构.assets/image-20240526013927929.png" alt="image-20240526013927929" style="zoom: 50%;" /> |
