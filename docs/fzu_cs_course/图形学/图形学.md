...
permalink: /fzu_cs_course/graphics/
---
> 本文为FZU庄一新老师的图形学(2023)课程笔记, 原文内容参考Games 101课程
>
> Author: [@亦瑾](https://github.com/yJader); (不愿透露姓名的)[@凌风](https://space.bilibili.com/234105594)
...
> 图形和成像中的采样失真出现的原因: 采样时出现<mark>“混叠”(aliasing)</mark>
...
3. 卷积定理(卷积的对偶性)
   - <mark>空间域中的卷积等于频域中的乘法，反之亦然</mark>
   - 选项 1：
     - 在空间域中通过卷积进行滤波
   - 选项 2：
     - 变换到频域（傅立叶变换）
...
1. MSAA: Multi-Sampling Anti-Aliasing <mark>多重采样反走样</mark> , **局部抗锯齿技术**, 只对选定的特定像素进行超采样(采样指的是计算颜色)
2. 实现方式
...
- <mark>注意</mark> : 三者此时并不是互相垂直的关系，因此需要进一步建立坐标系
...
1. <mark>投影变换</mark> : 将 3D 场景中的物体投影到 2D 投影面上
2. 投影的分类:
   - <img src="assets/image-20231103165119043.png" alt="image-20231103165119043" style="zoom:50%;" />
   - 如上图所示, 根据相机和光路进行区分, 分为<mark>正交投影</mark> 和<mark>透视投影</mark>

##### 正交投影（Orthographic Projection）

1. 正交投影:

   - 所有光线都是<mark>平行传播</mark>
...
3. 透视投影的计算: 将透视投影体(Frustum, 四棱台, 一个截去顶的四棱锥), 映射为立方体(这个立方体的面和棱台顶面相同), 那么我们的目的就是求得这样一个<mark>变换矩阵$M_{persp→ortho}$</mark> 实现映射
...
4. **总结**: 透视投影过程
   ...
   2. 在透视投影后，<mark>再利用一次正交投影</mark> 将空间投影成小立方体
...
<mark>优化镜面反射</mark> : 将反射方向与人眼观察方向夹角替换成如下图所示的一个半程向量和法线向量的夹角![image-20231026201037778](assets/image-20231026201037778.png)
...
2. 插值计算三角形中每个像素的颜色
   ...
   - 使用<mark>[重心坐标](# 6.2 重心坐标※)插值公式</mark> $c = αc_0 = βc_1+γc_2$
...
<mark>隐式曲面</mark> : 曲面上所有点满足的关系用**数学表达式**写出
...
<mark>显示曲面</mark> : 所有的点被**直接给出**(点集)，或者可以通过映射关系直接得到
...
- <mark>辐射通量(Radiant flux)</mark> : 单位时间的能量 Φ
...
- <mark>辐照度(Irradiance, $E$)</mark> : 在**单位时间**内，每个**单位面积**上接受到的光照的能量 (受照面单位面积上的辐射通量)
...
- <mark>单位立体角</mark> （方向性的强度）：定义球面上的方形单位面积为 dA，再除以半径的平方。
...
1. <mark>**辐射 Radince L**</mark> : Radiant Flux(Power) 它在每个单位立体角、每个投影的面上有多少
...
2. <mark>场辐射(File Radiance)</mark> 定义：空间中某一点在给定方向上的场辐射（亮度 luminance）是垂直于该方向的单位面积上每单位立体角的功率。
   <img src="./assets/image-20231030233632522.png" alt="image-20231030233632522" style="zoom:50%;" />

3. <mark>入射表面辐射(Incident Surface Radiance)</mark> : 到达表面的单位立体角的辐照度 (射线射向表面)
   <img src="./assets/image-20231030234451446.png" alt="image-20231030234451446" style="zoom:50%;" />
4. <mark>出射表面辐射度(Exiting Surface Radiance)</mark> : 是离开表面的每单位投影面积的强度 (表面向外射出)
...
<mark>渲染方程</mark> : $L_o(p, \omega_o) = L_e(p, \omega_o) + \int _{H^2}f_r(p, \omega_i→\omega_o)L_i(p, \omega_i)cos\theta d\omega_i$
...
- 用分布$f_r(\omega_i→\omega_o)$来描述, 即为<mark>表面反射率</mark>
...
1. 对于一个复杂的物体, 我们可以用一个简单的几何体(长方体/球体)将其包围, 如果射线与该几何体有交点, 再进一步求交, 这个几何体就叫做<mark>包围盒(Bounding Volumes)</mark>
...
2. 长方体包围盒求交
   ...
   - 总结: 长方体包围盒判断是否相交: <mark>t~enter~<t~exit~ && t~exit~>=0</mark>
3. 轴对齐平面求交
   - 对于普通的平面, ray 求交
...
2. 常见的空间划分方式
   ...
   - <mark>八叉树</mark> : 每一次把空间划分成八份，再递归的对子空间进行划分，直到满足一定的停止规则（比如某一次划分 8 个子空间中 7 个为空）
     - 缺点: 维数越高越复杂, n 维空间对应 2^n^叉树
   - <mark>KD 树</mark> : 每次把空间划分为两份，x，y，z 轴轮流切分，直到被切分节点中不存在物体则停止
   - <mark>BSP 树</mark> : 一种对空间二分的划分方法，每次选一个方向进行划分，与 KD 树的区别在于它不是横平竖直地切，且它会有划分的空间没有规则性，求交困难的缺点。（砍开二维用线，砍开三维用面，维度越高越复杂）
...
4. BVH 遍历
   ...
   - 作业中的实现
     ...
     Intersection BVHAccel::getIntersection(BVHBuildNode *node, const Ray &ray) const
     {
         ...
         if (node->left <mark> nullptr && node->right </mark>  nullptr)
         {
...
一种容易想到的方法是<mark>梯形法则</mark> , 将 a~b 均匀分为 n 个小梯形, 累加计算面积
...
2. 概率论复习
   - X: 随机变量。表示潜在值的分布
   - X~p(x): 随机变量 X 服从<mark>概率密度函数 p(x)</mark> （PDF）。描述随机变量在其定义域上取值的分布情况, 概率密度越大, X 落在该点附件的概率越大
   - <mark>概率分布函数</mark> $F(X) = \int_{-\infin}^{X}f(x)dx$, 表示随机变量落在-∞~X 的概率
